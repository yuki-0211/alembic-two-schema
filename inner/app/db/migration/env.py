...
from app.db.models import (Base, inner)
...
def run_migrations_online() -> None:
    ...
    with connectable.connect() as connection:
        ...
        if cmd_name == 'upgrade':
            connection.execute(f"set search_path to {INNER_SCHEMA}, public")
            version_table_schema = INNER_SCHEMA
        else:
            connection.execute(f"set search_path to {INNER_SCHEMA}")
        ...
        connection.dialect.default_schema_name = INNER_SCHEMA

        def include_object(object, name, type_, reflected, compare_to):
            if type_ == "table" and (object.schema != INNER_SCHEMA):  # type: ignore
                return False
            else:
                return True
